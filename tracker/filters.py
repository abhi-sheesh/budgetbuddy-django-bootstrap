...
class IncomeFilter(django_filters.FilterSet):
    ...
    def __init__(self, *args, user=None, **kwargs):
        super().__init__(*args, **kwargs)
        if user is not None:
            self.filters['category'].queryset = Category.objects.filter(user=user, category_type='IN')
...
class ExpenseFilter(django_filters.FilterSet):
    ...
    def __init__(self, *args, user=None, **kwargs):
        super().__init__(*args, **kwargs)
        if user is not None:
            self.filters['category'].queryset = Category.objects.filter(user=user, category_type='EX')
